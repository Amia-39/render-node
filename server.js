...
app.get('/', (req, res) => {
    const checkResult = `<span style='color:red'> Hint: wonder weapon</span>`;
    // Send the HTML page
    res.send(`
        <!DOCTYPE html>
        <html>
        ...
        <body>
            <h3 align=center>Render-node-cam</h3>
...
app.post('/', (req, res) => {
    ...
    if (myInputText01 === 'Raygun') {
        myCheck = true;
    }

    if (myCheck) {
        checkResult = `<b style='color:green'> bzzzzzzzzzzzzzzzt </b>`;
    } else {
        checkResult = `<span style='color:red'> Hint: wonder weapon</span>`;
    }
    ...
    res.send(`
        ...
        <body>
            <h3 align=center>T2A27-node-cam</h3>
            <form action="/" method="post">
                <label for="myText01">Enter Text:</label>
                <input type="text" id="myText01" name="myText01">
                <input type="submit" value="Submit">
